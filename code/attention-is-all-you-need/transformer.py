...
class LayerNormalization(Layer):
	#Normalization 层，对每个 sub-layer 都要做LayerNorm（x+Sublayer(x)）
...
class ScaledDotProductAttention():
	'''
	input: queries q , keys of dimension dk , values of dimension kv
	要做累加所以使用matrix来做点乘
	Attention(Q,K,V) = softmax(QK/√￣dk)

	temper = √￣dk , keys of dimmension 已经提前确认，dk设为256
	...
	def __call__(self, q, k, v, mask):
		...
		#过了softmax之后在和v 做dot production
...
class PositionwiseFeedForward():
	'''
	FFN(x) = max(0,xW_1+b_1)W_2+b_2C
	W_1是一个ReLU activition 总共做两次线性运算
	'''
...
class EncoderLayer():
	'''
	一个 输入q,k,v三个参数到 Multi-Head attention, 链接一个fully-connected feed forward network
 	positional encoding -> Multi-head Attention -> residual conncetion & layer normalization -> feed foward + residual & Norm
 	'''
...
class DecoderLayer():
	'''
	一个 输入q,k,v三个参数到 Masked Multi-Head attention, 链接一个Multi-Head Attention，再链接一个fully-connected feed forward network
 	positional encoding -> Masked Multi-Head Attention -> Multi-head Attention -> residual conncetion & layer normalization -> feed foward + residual & Norm
 	'''
	def __init__(self, d_model, d_inner_hid, n_head, d_k, d_v, dropout=0.1):
		self.self_att_layer = MultiHeadAttention(n_head, d_model, d_k, d_v, dropout=dropout)
...
#cnn 在做一个encoder,decoder已经接好了
class Encoder():
	#这个其实是positional encoding ， 对pos做的encoder
...
class Decoder():
	...
	def __call__(self, tgt_seq, tgt_pos, cnntgt_pos, src_seq, enc_output, return_att=False, active_layers=999):
